...
interface StunningDataPopupProps {
  isOpen: boolean;
  onClose: () => void;
  title: string;
  data: any;
...
const StunningDataPopup = ({
  ...
  const getCategoryGradient = (cat: string) => {
    switch (cat) {
      case 'lead': return 'from-blue-600 via-blue-700 to-indigo-800';
      case 'customer': return 'from-emerald-600 via-green-700 to-teal-800';
      case 'feedback': return 'from-purple-600 via-violet-700 to-purple-800';
      case 'grievance': return 'from-red-600 via-rose-700 to-pink-800';
      case 'report': return 'from-orange-600 via-amber-700 to-yellow-800';
      default: return 'from-purple-600 via-purple-700 to-indigo-800';
    }
  };

  const renderField = (field: any) => {
    const value = editMode ? editData[field.key] : data[field.key];
    const FieldIcon = field.icon || getFieldIcon(field.type, field.key);
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <Badge className={`px-4 py-2 font-bold text-sm rounded-full ${getBadgeStyle(value)}`}>
                    {value || "N/A"}
                  </Badge>
                </div>
              </div>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[95vh] overflow-hidden flex flex-col p-0 gap-0 rounded-3xl border-4 border-white shadow-2xl">
        ...
        <div className={`px-8 py-8 bg-gradient-to-br ${getCategoryGradient(category)} relative overflow-hidden`}>
          ...
          <div className="absolute top-1/4 right-1/3 w-1 h-1 bg-white/60 rounded-full animate-pulse"></div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-sm font-normal text-white/90">
                      {editMode ? 'Edit Mode - Make your changes' : 'View Mode - All details'}
                    </span>
                  </div>
                </div>
              </DialogTitle>
        ...
        {/* Content Area with stunning design */}
...
export default StunningDataPopup;
